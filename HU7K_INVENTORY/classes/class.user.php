...
class User{
	...
	public function new_user($firstname,$lastname,$access,$email,$password){
		
		$data = [
			[$firstname,$lastname,$access,$email,$password],
		];
		$stmt = $this->conn->prepare("INSERT INTO tbl_users (user_email, user_password, user_lastname, user_firstname, user_access) VALUES (?,?,?,?,?)");
		try {
